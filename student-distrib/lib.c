...
static uint8_t tmnl_attrib_2[3] = {ATTRIB_TERM1, ATTRIB_TERM2, ATTRIB_TERM3};
...
void clear(void) {
    ...
    screen_x = 0;
    screen_y = 0;
    update_cursor(screen_x, screen_y,active_terminal);
}
 ...
 * Inputs: x, y (position)
 * Return Value: none
 * Function: Updates screen cursor to position */
void update_cursor(int x, int y, int terminal)
{
    ...
    tmnl_block[terminal].pos_x = x;
    tmnl_block[terminal].pos_y = y;
}


/* void get_cursor_x(void);
 * Inputs: void
 * Return Value: cursor x-position
 * Function: Returns cursor x-position */
 ...
 * Inputs: void
 * Return Value: cursor y-position
 * Function: Returns cursor y-position */
    ...
    if(active_terminal==exec_terminal)
    {
        if(c == '\n' || c == '\r') {
            if(screen_y >= NUM_ROWS - 1){
                memcpy(video_mem,video_mem+(2*NUM_COLS),(NUM_COLS*(NUM_ROWS-1)*2));
                int i = ((NUM_ROWS-1)*NUM_COLS*2);
                for(;i<NUM_ROWS*NUM_COLS*2;i+=2)
                {
                    video_mem[i] = ' ';
                    video_mem[i+1]=tmnl_attrib_2[active_terminal];
                }
                screen_x = 0;
                update_cursor(screen_x,screen_y,active_terminal);
                return;
        ...
        }else if(c==BACKSPACE)
        {
            ...
            if((screen_x==start_x) && (screen_y==start_y)){
                return;
            } 
        ...
        }else if(c == F1 || c == F2 || c == F3)
        {
            ...
            *(uint8_t *)(video_mem + ((NUM_COLS * screen_y + screen_x) << 1) + 1) = tmnl_attrib_2[active_terminal];

            //temp protecting page faults
        ...
        if(c == '\n' || c == '\r') {
            ...
            if(tmnl_posy >= NUM_ROWS-1){
                ...
                for(i=((NUM_ROWS-1)*NUM_COLS*2); i<NUM_ROWS*NUM_COLS*2; i+=2){
                    tmnl_vidmem[i] = ' ';
                    tmnl_vidmem[i+1] = tmnl_attrib_2[exec_terminal];
                }
            }
            // Move position to new line
            tmnl_block[exec_terminal].pos_x = 0;
            if(tmnl_block[exec_terminal].pos_y!=NUM_ROWS-1)
            ...
            *((uint8_t *)(tmnl_vidmem + ((NUM_COLS*tmnl_posy + tmnl_posx) << 1))) = c;
            // if(c==' ')
            //     *((uint8_t *)(tmnl_vidmem + ((NUM_COLS*tmnl_posy + tmnl_posx) << 1))) = '!';
            *((uint8_t *)(tmnl_vidmem + ((NUM_COLS*tmnl_posy +tmnl_posx) << 1) + 1)) = tmnl_attrib_2[exec_terminal];
